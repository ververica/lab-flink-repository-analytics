...
/** Integration test for {@link LastValueStringArrayAggFunction}. */
public class LastValueStringArrayAggFunctionITCase extends AbstractTableTestBase {
  ...
  private List<Row> executeSql() throws Exception {
    TableResult resultTable =
        tEnv.executeSql(
            "SELECT\n"
                + "  name,\n"
                + "  LastValueStringArrayAggFunction(aliases)\n"
                + "FROM input\n"
                + "GROUP BY name");
